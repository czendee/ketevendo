...
public class CRUDSocioGetOneSocio extends AsyncTask<String, Void, List<DataDescriptorSocio>> {
...
//            adpt.notifyDataSetChanged();
        System.out.println("CRUDSocioGetOneSocio onPostExecute paso 1");

        //cylce through List DataDescriptorRobot result, y ponerlo en lista de ArrayTask
        if(result!=null) {
            for (DataDescriptorSocio f : result) {
                socioStr=f.getName() + " - " + f.getMobile() + " - " + f.getPin() + "-" + f.getStatus() + "-";// api socios
                socioobtenido.setId(f.getId());
                socioobtenido.setInternalId(f.getInternalId());
                socioobtenido.setMobile(f.getMobile());
                socioobtenido.setPin(f.getPin());
                socioobtenido.setStatus(f.getStatus());
                System.out.println("CRUDSocioGetOneSocio onPostExecute paso 2");
                break;
            }
            ...
            System.out.println("CRUDSocioGetOneSocio onPostExecute paso 2");
//            updateMap();
            System.out.println("CRUDSocioGetOneSocio onPostExecute paso 3");
        }
    }
    @Override
